...
export default class EditPopup extends React.Component {
  ...
  handleNameChange = (e) => {
    this.setState((prevState) => {
      return { task: { ...prevState.task, name: e.target.value}};
    });
  }

  handleDecriptionChange = (e) => {
    this.setState((prevState) => {
      return { task: { ...prevState.task, description: e.target.value }};
    });
  }
  ...
  handleCardEdit = () => {
    fetch('PUT', window.Routes.api_v1_task_path(this.props.cardId, {format: 'json'}), {
      ...
      if (response.statusText === 'OK') {
        ...
        const myAlert = alert;
        myAlert(`Update failed! ${response.status} - ${response.statusText}`);
      }
    });
  }
    ...
    fetch('DELETE', window.Routes.api_v1_task_path(this.props.cardId, { format: 'json' }))
      .then( response => {
        if (response.statusText === 'OK') {
          ...
          const myAlert = alert;
          myAlert(`DELETE failed! ${response.status} - ${response.statusText}`);
        }
      });
  }
